...
\begin{definition}\label{def:flag-bundle} If $E \to X$ is a real vector bundle, we denote by $\Fl(E)$ the \emph{flag bundle}, whose fibers $\Fl(E)_x$ consist of all the orthonormal flags in $E_x$. We claim that $\Fl(E)$ is a principal $O(k)$-bundle over $X$.
\end{definition}

\begin{exercise}\label{exer:flag-bundle-is-associated} 
...
\end{align*}
we have that $\Fl(E)$ corresponds to the vector bundle $E \to X$.
\end{exercise}
This tells us that:
\begin{itemize}
    \item a vector bundle can be recovered by its flag bundle
    \item every principal $O(k)$-bundle is the flag bundle of some vector space, uniquely determined up to isomorphism
...
\begin{definition}\label{def:flag-bundle}
Let $\pi \colon V \to X$ be a rank $r$ complex vector bundle, defined by the data of a cover $\left\{ U_i \right\}$ and trivializations $\phi_i \colon U_i \times \C^r \xto{\sim} \pi^{-1}(U_i)$. For a fiber $V_x$ over a point $x\in X$, denote by $F(V_x)$ the set of flags of the vector space $V_x$. This receives a free transitive $\GL_r(\C)$-action, but has no topology a priori. Denote by $F(V) = \amalg_{x\in X}F(V_x)$, and endow it with the final topology coming from the inclusions $\phi_i$.
\end{definition}
