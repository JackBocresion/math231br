...
\begin{align*}
    \Omega^k_{\dR} = \Sec(-,\wedge^k T^\ast M)
\end{align*}
the sheaf of sections of the $k$th exterior power of the cotangent bundle.
\end{definition}

\begin{example} We have that $\Omega^0_\dR$ is the sheaf of real-valued differentiable functions on $M$.
\end{example}
...
In general we define the \emph{exterior derivative} of a differential $k$-form
\begin{align*}
...
\begin{align*}
    ...
    &= f(tx_1, \ldots, tx_n) \left( dx_I + \sum_{j=1}^n (-1)^j dx_{I\minus\{j\}} \right).
\end{align*}
...
\end{align*}
To verify this equals $\omega$, it suffices to check that
\begin{align*}
    f(x_1, \ldots, x_n) = \sum_{i=1}^n \int_0^1 \frac{\partial f}{\partial x_i}(tx_1, \ldots, tx_n)dt
\end{align*}
This is a standard version of the fundamental theorem of calculus that holds in star-shaped domains; see for instance \cite[I.1.4]{tuIntroductionManifolds2011}).
\end{proof}
