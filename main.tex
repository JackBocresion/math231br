...
Note that baked into the definition is the existence of a universal bundle. By the Yoneda lemma, the homotopy class of the identity map on $B$ is mapped to some bundle $\eta \colon E \to B$ in $\check{H}^1(B,G_c)$, and every element in $[X,B]$ is the pullback of $\eta$ for every $X$. So to study and classify principal $G$-bundles, we should start by classifying a universal one. The \emph{classification theorem} of Dold gives us a lovely way to do this:
...
\begin{proof}[Sketch]
First suppose $\eta \colon E \to B$ is contractible, and take some other principal $G$-bundle $\eta \colon V \to X$. We want to find a map $f \colon X \to B$ for which $V\cong f^\ast E$.
...
For the other direction, suppose $\eta$ is universal. If we can prove the existence of a contractible numerable principal bundle (\cite[\S8]{doldPartitionsUnityTheory1963}), call it $C \to X$, then it is also universal by the previous direction, so we have a composite
\[ \begin{tikzcd}
...
\begin{remark} A mild modification of the argument in the proof of \Cref{thm:classification-thm-principal-G-bundles} shows that universal classifying spaces of principal $G$-bundles are unique up to homotopy equivalence.
\end{remark}
